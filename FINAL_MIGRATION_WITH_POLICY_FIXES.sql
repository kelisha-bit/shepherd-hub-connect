-- Final Complete Database Schema Migration
-- This migration handles existing policies and creates all missing tables
...
BEGIN
  FOR table_name IN 
    ...
    AND tablename IN (
      ...
      'small_groups', 'income_categories', 'incomes',
...
-- Insert default income categories (without ON CONFLICT)
DO $$
BEGIN
  -- Insert only if not exists
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Tithes') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Tithes', 'Regular tithes from members');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'General Offering') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('General Offering', 'General offerings during services');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Prophetic seed') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Prophetic seed', 'Prophetic seed offerings');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Thanksgiving') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Thanksgiving', 'Thanksgiving offerings');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Pledge') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Pledge', 'Pledge payments');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Wednesday offering') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Wednesday offering', 'Wednesday service offerings');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.income_categories WHERE name = 'Others') THEN
    INSERT INTO public.income_categories (name, description) VALUES ('Others', 'Other income sources');
  END IF;
END $$;

-- Insert default expense categories (without ON CONFLICT)
DO $$
BEGIN
  -- Insert only if not exists
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Utilities') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Utilities', 'Electricity, water, gas, internet');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Maintenance') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Maintenance', 'Building and equipment maintenance');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Staff Salaries') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Staff Salaries', 'Employee and staff compensation');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Office Supplies') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Office Supplies', 'Office and administrative supplies');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Outreach Programs') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Outreach Programs', 'Community outreach and missions');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Events') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Events', 'Special events and celebrations');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Technology') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Technology', 'Software, hardware, and IT services');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Insurance') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Insurance', 'Property and liability insurance');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Taxes') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Taxes', 'Property taxes and other taxes');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_categories WHERE name = 'Other') THEN
    INSERT INTO public.expense_categories (name, description) VALUES ('Other', 'Miscellaneous expenses');
  END IF;
END $$;
...
DO $$
BEGIN
  -- Drop all existing policies for the tables we're creating
  ...
  DROP POLICY IF EXISTS "Everyone can view sermons" ON public.sermons;
  ...
  DROP POLICY IF EXISTS "Admins can manage incomes" ON public.incomes;
  DROP POLICY IF EXISTS "Admins can manage expense categories" ON public.expense_categories;
  DROP POLICY IF EXISTS "Admins can manage expenses" ON public.expenses;
  DROP POLICY IF EXISTS "Admins can manage financial goals" ON public.financial_goals;
  
  DROP POLICY IF EXISTS "Authenticated users can view attendance counts" ON public.event_attendance_counts;
  DROP POLICY IF EXISTS "Admins can manage attendance counts" ON public.event_attendance_counts;
  
  DROP POLICY IF EXISTS "Users can view profile pictures" ON public.profile_pictures;
  DROP POLICY IF EXISTS "Users can manage their own profile pictures" ON public.profile_pictures;
  
  DROP POLICY IF EXISTS "Users can view event images" ON public.event_images;
  DROP POLICY IF EXISTS "Admins can manage event images" ON public.event_images;
  
  DROP POLICY IF EXISTS "Users can view hero images" ON public.hero_images;
